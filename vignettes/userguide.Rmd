...
Thus, downstream code just needs to accept a pointer, cast it to a `BiocNeighbors::Prebuilt`, and then use the `index` therein for nearest-neighbor searches.
...
- Implement a `SomeNewParam` S4 class that inherits from `BiocNeighborParam`.
- Implement a `buildIndex()` method, dispatching on a `matrix` type for `X=` and a `SomeNewParam` type for `BNPARAM=`.
  This should call into C++ and return an external pointer to an instance of a `BiocNeighbors::Prebuilt`.
