...
Once we've built an index, we can save it to files on disk with the `saveIndex()` function.

```{r}
savedir <- tempfile()
dir.create(savedir)
saveIndex(prebuilt, savedir)
list.files(savedir, recursive=TRUE)
```

This can be reloaded in a new R session with `loadIndex()`.
The aim is to save some time by avoiding a rebuild of the index from the input data.

```{r}
reloaded <- loadIndex(savedir)
reloaded
```

However, keep in mind that files generated by `saveIndex()` are not guaranteed to be portable across architectures, compilers, or even versions of `r self`.
...
- Implement methods for `findKnnFromIndex()`, `findDistanceFromIndex()`, `queryKnnFromIndex()`, `queryDistanceFromIndex()` and `saveIndex()`.
  Optionally methods can be implemented for `findNeighborsFromIndex()` and `queryNeighborsFromIndex()`.
- Register a loading function via `registerLoadIndexFunction()`, typically in the package's `.onLoad()`.
...
- Register a C++ function for loading the index via the external pointer returned by `getLoadPrebuiltRegistry()`.
  Also register the class constructor in `registerLoadGenericIndexClass()`.
  Both of these are typically done in the package's `.onLoad()`.

No new methods are required for `findKnnFromIndex()`, `queryNeighborsFromIndex()`, etc. as a `BiocNeighborGenericIndex` method is already available for each generic.
