...
The `r Biocpkg("kmknn")` package provides an implementation of the [Annoy](https://github.com/spotify/annoy) (Approximate Nearest Neighbors Oh Yeah) method based on C++ code in the `CRANpkg("RcppAnnoy")` package.
The aim is to provide an approximate method to complement the exact KMKNN algorithm `r Biocpkg("kmknn", vignette="kmknn.html", label="described previously")`.
