...
# library(BiocNeighbors); library(testthat); source("test-range-find-kmknn.R")
...
test_that("rangeFindKmknn() behaves correctly with parallelization", {
    ...
    out1 <- rangeFindKmknn(X, threshold=d, BPPARAM=MulticoreParam(2))
...
test_that("rangeFindKmknn() behaves with variable distances", {
    ...
    expect_identical_re(out, rangeFindKmknn(X, threshold=d, BPPARAM=MulticoreParam(3)))
})
