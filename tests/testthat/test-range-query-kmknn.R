...
# library(BiocNeighbors); library(testthat); source("test-range-query-kmknn.R")
...
test_that("rangeQueryKmknn() behaves correctly with parallelization", {
    ...
    out1 <- rangeQueryKmknn(X, Y, threshold=d, BPPARAM=MulticoreParam(2))
...
test_that("rangeQueryKmknn() behaves with variable distances", {
    ...
    expect_identical_re(out, rangeQueryKmknn(X, Y, threshold=d, BPPARAM=MulticoreParam(3)))
})
