...
test_that("queryExhaustive() behaves correctly with queries", {
    ...
    for (ndim in c(1, 5, 10, 20)) {
        for (k in c(1, 5, 20)) { 
            ...
            out <- queryExhaustive(X, k=k, query=Y)
...
set.seed(1002)
test_that("queryExhaustive() works correctly with subsetting", {
    nobs <- 1000
    nquery <- 93
    ndim <- 21
    k <- 7
    ...
    Y <- matrix(runif(nquery * ndim), nrow=nquery)
    ref <- queryExhaustive(X, Y, k=k)

    i <- sample(nquery, 20)
    sub <- queryExhaustive(X, Y, k=k, subset=i)
...
set.seed(1003)
test_that("queryExhaustive() behaves correctly with alternative options", {
    nobs <- 1000
    nquery <- 100
    ndim <- 10
    k <- 5
    ...
    X <- matrix(runif(nobs * ndim), nrow=nobs)
    Y <- matrix(runif(nquery * ndim), nrow=nquery)
    out <- queryExhaustive(X, Y, k=k)
    
    # Checking what we extract.
    out2 <- queryExhaustive(X, Y, k=k, get.distance=FALSE)
    expect_identical(out2$distance, NULL)
    expect_identical(out2$index, out$index)

    out3 <- queryExhaustive(X, Y, k=k, get.index=FALSE)
    expect_identical(out3$index, NULL)
    expect_identical(out3$distance, out$distance)
  
    # Checking precomputation.
    pre <- buildExhaustive(X)
    out4 <- queryExhaustive(query=Y, k=k, precomputed=pre) # no need for X!
    expect_identical(out4, out)

    # Checking transposition.
    out5 <- queryExhaustive(X, k=k, query=t(Y), transposed=TRUE)
    expect_identical(out5, out)
})

set.seed(1003001)
test_that("queryExhaustive() works with Manhattan distances", {
    ...
    for (ndim in c(1, 5, 10)) {
        for (k in c(1, 5, 20)) { 
            ...
            out <- queryExhaustive(X, k=k, query=Y, distance="Manhattan")
...
test_that("queryExhaustive() works to only obtain the last distance", {
    ...
    for (ndim in c(1, 5, 10)) {
        for (k in c(1, 5, 20)) { 
            ...
            ref <- queryExhaustive(X, k=k, query=Y)
            out <- queryExhaustive(X, k=k, query=Y, last=1)
            expect_identical(out$distance, ref$distance[,k,drop=FALSE])
            expect_identical(out$index, ref$index[,k,drop=FALSE])

            ref <- queryExhaustive(X, k=k, query=Y, distance="Manhattan")
            out <- queryExhaustive(X, k=k, query=Y, last=1, distance="Manhattan")
            expect_identical(out$distance, ref$distance[,k,drop=FALSE])
            expect_identical(out$index, ref$index[,k,drop=FALSE])
        }
    }
...
set.seed(100301)
test_that("queryExhaustive() behaves correctly with parallelization", {
...
test_that("queryExhaustive() raw output behaves correctly", {
    ...
    pre <- buildExhaustive(X)
    out <- queryExhaustive(query=Y, k=k, precomputed=pre, raw.index=TRUE)
    ref <- queryExhaustive(query=Y, X=t(bndata(pre)), k=k)
    expect_identical(out, ref)

    # Behaves with subsetting.
    i <- sample(nquery, 20)
    out <- queryExhaustive(query=Y, k=k, precomputed=pre, raw.index=TRUE, subset=i)
    ref <- queryExhaustive(query=Y, X=t(bndata(pre)), k=k, subset=i)
    expect_identical(out, ref)
...
set.seed(1004)
test_that("queryExhaustive() behaves correctly with silly inputs", {
    nobs <- 1000
	nquery <- 100
    ndim <- 10
    X <- matrix(runif(nobs * ndim), nrow=nobs)
    ...
    expect_error(queryExhaustive(X, Y, k=0), "positive")
    expect_error(queryExhaustive(X, Y, k=-1), "positive")

    # What happens when there are more NNs than k.
    restrict <- 10
    expect_warning(out <- queryExhaustive(X[seq_len(restrict),], Y, k=20), "capped")
    expect_warning(ref <- queryExhaustive(X[seq_len(restrict),], Y, k=restrict), NA)
    expect_equal(out, ref)

    # What happens when there are no dimensions.
    out <- queryExhaustive(X[,0], Y[,0], k=20)
    ...
    expect_error(queryExhaustive(X, k=20, query=Z), "dimensionality")

    # What happens when we request raw.index without precomputed.
    expect_error(queryExhaustive(X, Y, k=20, raw.index=TRUE), "not valid")

    # What happens when the query is not, strictly a matrix.
    AA <- data.frame(Y)
    colnames(AA) <- NULL
    expect_equal(queryExhaustive(X, Y, k=20), queryExhaustive(X, AA, k=20))

    # What happens with nothing.
    expect_identical(queryExhaustive(X, Y, k=10, get.distance=FALSE, get.index=FALSE), list())
})
