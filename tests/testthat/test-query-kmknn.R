# Tests queryKmknn().
# library(BiocNeighbors); library(testthat); source("test-query-kmknn.R")
...
test_that("queryKmknn() behaves correctly with parallelization", {
    ...
    out1 <- queryKmknn(X, Y, k=k, BPPARAM=MulticoreParam(2))
...
test_that("queryKmknn() raw output behaves correctly", {
    ...
    ref <- queryKmknn(query=Y, X=t(KmknnIndex_clustered_data(pre)), k=k)
    expect_identical(out, ref)

    # Behaves with subsetting.
    i <- sample(nquery, 20)
    out <- queryKmknn(query=Y, k=k, precomputed=pre, raw.index=TRUE, subset=i)
    ref <- queryKmknn(query=Y, X=t(KmknnIndex_clustered_data(pre)), k=k, subset=i)
    expect_identical(out, ref)
